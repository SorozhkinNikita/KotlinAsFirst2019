...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (str in subSet) map[str] = 0
    for (line in File(inputName).readLines())
        for (str in subSet) {
            var startIndex = 0
            val lowLine = line.toLowerCase()
            val lowStr = str.toLowerCase()
            var ind = lowLine.indexOf(lowStr, startIndex)
            while (ind != -1) {
                map[str] = map[str]!! + 1
                startIndex = ind + 1
                ind = lowLine.indexOf(lowStr, startIndex)
            }
        }
    return map
}
...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    for (line in File(inputName).readLines()) {
        ...
        lengthLines.add(lengthWords)
        maxLength = max(maxLength, lengthWords)
    ...
    File(outputName).bufferedWriter().use {
        for ((index, line) in text.withIndex()) {
            ...
            val delta = maxLength - lengthLines[index]
            val countPosSpaces = line.size - 1
            val divSpaces = delta / countPosSpaces
            var modSpaces = delta % countPosSpaces
