...
fun bisectorByPoints(a: Point, b: Point): Line {
    val angle = if (a.x == b.x) PI / 2 else abs((atan((a.y - b.y) / (a.x - b.x)) + PI) % PI)
...
fun findNearestCirclePair(vararg circles: Circle): Pair<Circle, Circle> {
    val c = circles.toList()
    require(c.size >= 2)
    var resCircles = Pair(c[0], c[1])
    var resDis = c[0].distance(c[1])
    for (i in 0..c.size - 2) {
        for (j in i + 1 until c.size) {
            val d = c[i].distance(c[j])
            if (resDis > d) {
                resCircles = Pair(c[i], c[j])
                resDis = d
            }
        }
    }
...
fun circleWithOneFixedPoint(point: Point, points: List<Point>): Circle {
    ...
    for (i in 1 until points.size) {
        if (!res.contains(points[i])) res = circleWithTwoFixedPoint(point, points[i], points.take(i))
    }
    return res
}
